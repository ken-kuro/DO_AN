...
export class IntegrationsService {
  ...
  async linkedIn(user: User, path: string): Promise<ResumeEntity> {
    ...
    try {
      ...
      return this.resumeService.import(resume, user);
    } catch {
  ...
  async jsonResume(user: User, path: string) {
    try {
  ...
  async reactiveResume(user: User, path: string): Promise<ResumeEntity> {
    try {
      const jsonResume = JSON.parse(await readFile(path, 'utf8'));
  ...
  async reactiveResumeV2(user: User, path: string): Promise<ResumeEntity> {
    try {
      const jsonResume = JSON.parse(await readFile(path, 'utf8'));
